...
# Rotates an image by a user-defined angle (angle is in radians)
...
def find_card_in_img(im, debug):
    ...
    for contour in contours:
        ...
        if area_box>=min_shape:
            ...
            #debug
            cv2.drawContours(im_grad_th_color, [box], -1, (0, 0, 255), 2)  # on peut drawer le bounding box avec drawContours
            ...
            if (min(L1/L2,L2/L1)>=real_card_ratio-0.05) and min(L1/L2,L2/L1)<=real_card_ratio+0.05:
                # print(L1/L2)
                cv2.drawContours(im_grad_th_color, [box], -1, (255, 0, 255), 2)  # on peut drawer le bounding box avec drawContours
                #draw all candidate contours
                cv2.drawContours(im_contour_candidates, [box], -1, 255, 2)  # on peut drawer le bounding box avec drawContours
    ...
    if len(contours)>=2:
        ...
        for contour in contours:
            ...
            if len(contours)>=2:
                L1 = distance(box[0], box[1])
                L2 = distance(box[1], box[2])
                if (L1 >= 0.67 * L2 and L1 <= 0.77 * L2) or (L2 >= 0.67 * L1 and L2 <= 0.77 * L1):  # ratio officiel 0.72
